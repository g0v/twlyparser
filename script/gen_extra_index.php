...
function Gazettes() {
  if(isset($GLOBALS['GAZETTES'])) return $GLOBALS['GAZETTES'];

  $content = file_get_contents("data/gazettes.json");
  $content_st = json_decode($content, true);
  return $content_st;
}
...
function GazettesMapping($gazettes) {
  if(isset($GLOBALS['GAZETTES_MAPPING'])) return $GLOBALS['GAZETTES_MAPPING'];

  $result = array();

  foreach($gazettes as $each_key => $each_val) {
    $mapping = $each_val['year'] . $each_val['vol'];
    $result[$mapping] = $each_key;
  }
  ...
  $GLOBALS['GAZETTES_MAPPING'] = $result;
  return $result;
}
...
function ProcessFile($filename) {
  ...
  $gazettes = Gazettes();
  $gazettes_mapping = GazettesMapping($gazettes);

  $gazette = Gazette($filename_infix, $gazettes_mapping);
